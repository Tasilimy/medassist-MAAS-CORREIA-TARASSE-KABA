...
BEGIN
  ...
  IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'gender_ref') THEN
    RAISE NOTICE '[✓] Test 1 : Table gender_ref existe';
  ELSE
    RAISE NOTICE '[✗] Test 1 ÉCHOUÉ : Table gender_ref n''existe pas';
  ...
  IF EXISTS (
    ...
    RAISE NOTICE '[✓] Test 2 : Colonne gender_new existe dans patients';
  ELSE
    RAISE NOTICE '[✗] Test 2 ÉCHOUÉ : gender_new manquant';
    v_failed_count := v_failed_count + 1;
  END IF;
  ...
  IF (SELECT COUNT(*) FROM gender_ref) = 4 THEN
    RAISE NOTICE '[✓] Test 3 : 4 valeurs de référence (M, F, NB, U)';
  ELSE
    RAISE NOTICE '[✗] Test 3 ÉCHOUÉ : Nombre incorrect de valeurs';
    v_failed_count := v_failed_count + 1;
  END IF;
  ...
  IF (SELECT COUNT(*) FROM patients WHERE gender_new IS NULL) = 0 THEN
    RAISE NOTICE '[✓] Test 4 : Tous les patients ont une valeur de gender_new';
  ELSE
    RAISE NOTICE '[✗] Test 4 ÉCHOUÉ : % patients avec gender_new NULL',
  ...
  IF (SELECT COUNT(*) FROM patients WHERE gender_new NOT IN ('M', 'F', 'NB', 'U')) = 0 THEN
    RAISE NOTICE '[✓] Test 5 : Toutes les valeurs de gender_new sont valides';
  ELSE
    RAISE NOTICE '[✗] Test 5 ÉCHOUÉ : Valeurs invalides détectées';
    v_failed_count := v_failed_count + 1;
  END IF;
  ...
  IF EXISTS (
    ...
    RAISE NOTICE '[✓] Test 6 : FK vers gender_ref existe';
  ELSE
    RAISE NOTICE '[⚠] Test 6 : Avertissement - FK vers gender_ref non trouvée';
  END IF;
  ...
  IF EXISTS (
    ...
    RAISE NOTICE '[✓] Test 7 : Colonne gender (V1) toujours présente';
  ELSE
    RAISE NOTICE '[✗] Test 7 ÉCHOUÉ : gender supprimé';
    v_failed_count := v_failed_count + 1;
  END IF;
