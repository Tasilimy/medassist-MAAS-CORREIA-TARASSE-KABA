...
BEGIN
  ...
  IF NOT EXISTS (
    ...
    RAISE NOTICE '[OK] Test 1 : Anciennes colonnes adresse supprimées';
  ELSE
    RAISE NOTICE '[ERREUR] Test 1 ÉCHOUÉ : Colonnes adresse encore présentes';
  ...
  IF NOT EXISTS (
    ...
    RAISE NOTICE '[OK] Test 2 : Colonne doctor_name supprimée';
  ELSE
    RAISE NOTICE '[ERREUR] Test 2 ÉCHOUÉ : doctor_name toujours présent';
    v_failed_count := v_failed_count + 1;
  END IF;
  ...
  IF EXISTS (
    ...
    RAISE NOTICE '[OK] Test 3 : gender_new renommé en gender';
  ELSE
    RAISE NOTICE '[ERREUR] Test 3 ÉCHOUÉ : gender introuvable';
    v_failed_count := v_failed_count + 1;
  END IF;
  ...
  IF (SELECT COUNT(*) FROM patients WHERE gender IS NULL) = 0 THEN
    RAISE NOTICE '[OK] Test 4 : Tous les patients ont une valeur de gender';
  ELSE
    RAISE NOTICE '[ERREUR] Test 4 ÉCHOUÉ : % patients avec gender NULL',
  ...
  IF (SELECT COUNT(*) FROM pg_trigger 
    ...
    RAISE NOTICE '[OK] Test 5 : Triggers de synchronisation supprimés';
  ELSE
    RAISE NOTICE '[AVERTISSEMENT] Test 5 : Avertissement - Certains triggers restent (normal si pas de CONTRACT)';
  END IF;
  ...
  IF (SELECT COUNT(*) FROM consultations WHERE doctor_id IS NOT NULL) > 0 THEN
    RAISE NOTICE '[OK] Test 6 : doctor_id présent et utilisé (% consultations)',
      (SELECT COUNT(*) FROM consultations WHERE doctor_id IS NOT NULL);
  ELSE
    RAISE NOTICE '[ERREUR] Test 6 ÉCHOUÉ : Aucune consultation avec doctor_id';
    v_failed_count := v_failed_count + 1;
  END IF;
  ...
  IF (SELECT COUNT(*) FROM patients) > 0 
    ...
    RAISE NOTICE '[OK] Test 7 : Toutes les données intactes';
  ELSE
    RAISE NOTICE '[ERREUR] Test 7 ÉCHOUÉ : Données manquantes';
    v_failed_count := v_failed_count + 1;
  END IF;
  ...
  IF (SELECT COUNT(DISTINCT patient_id) FROM addresses WHERE is_primary = TRUE) > 0 THEN
    RAISE NOTICE '[OK] Test 8 : Adresses primaires validées';
  ELSE
    RAISE NOTICE '[AVERTISSEMENT] Test 8 : Avertissement - Pas d''adresse primaire';
  END IF;
  ...
  IF EXISTS (SELECT 1 FROM information_schema.views WHERE table_name = 'consultations_legacy') THEN
    RAISE NOTICE '[OK] Test 9 : Vue consultations_legacy créée';
  ELSE
    RAISE NOTICE '[AVERTISSEMENT] Test 9 : Vue consultations_legacy introuvable';
  END IF;
  ...
  IF EXISTS (
    ...
    RAISE NOTICE '[OK] Test 10 : Index présent sur addresses';
  ELSE
    RAISE NOTICE '[AVERTISSEMENT] Test 10 : Avertissement - Index manquant';
  END IF;
